...
public:
    ...
    void wire()
    {
        backend->onReadyRead = [this]() {
            if (cbs.onReadyRead)
                cbs.onReadyRead();
        };
        backend->onBytesWritten = [this](std::int64_t n) {
            if (cbs.onBytesWritten)
                cbs.onBytesWritten(n);
        };
        backend->onError = [this](SerialError e) {
            if (cbs.onError)
                cbs.onError(e);
        };
    }
...
std::int32_t SerialPort::baudRate(Directions) const
{
    return impl().baudRate_;
}
...
bool SerialPort::clear(Directions d)
{
    return impl().backend->clear(d);
}
bool SerialPort::flush()
{
    return impl().backend->flush();
}
std::int64_t SerialPort::bytesAvailable() const
{
    return impl().backend->bytesAvailable();
}
std::int64_t SerialPort::bytesToWrite() const
{
    return impl().backend->bytesToWrite();
}
SerialError SerialPort::error() const
{
    return impl().backend->lastError();
}
void SerialPort::clearError()
{
    impl().backend->clearError();
}
