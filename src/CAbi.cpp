...
#ifdef __ANDROID__
namespace cps { namespace detail { cps::SerialError androidInit(void* java_vm, void* context); } }
#endif
...
extern "C" CPS_API void cps_free_ports(cps_PortInfo* ports) {
    if (!ports) return;
    PortInfoBlock* block = reinterpret_cast<PortInfoBlock*>(
        reinterpret_cast<char*>(ports) - offsetof(PortInfoBlock, entries));
...
extern "C" CPS_API cps_SerialPort* cps_serial_new(const char* port_name) {
    ...
    if (port_name) h->port.setPortName(port_name);
    return h;
}
...
extern "C" CPS_API cps_SerialError cps_serial_set_baud_rate(cps_SerialPort* p, int rate, cps_Directions) {
    if (!p) return CPS_ERR_UNKNOWN;
...
extern "C" CPS_API cps_SerialError cps_serial_set_flow_control(cps_SerialPort* p, cps_FlowControl flow) {
    if (!p) return CPS_ERR_UNKNOWN;
...
extern "C" CPS_API cps_SerialError cps_serial_clear(cps_SerialPort* p, cps_Directions dir) {
    if (!p) return CPS_ERR_UNKNOWN;
    return p->port.clear(static_cast<cps::Directions>(dir)) ? CPS_ERR_NONE
...
#else
    (void)java_vm; (void)android_context;
...
extern "C" CPS_API const char* cps_error_string(cps_SerialError err) {
    switch (err) {
        case CPS_ERR_NONE:                   return "no error";
        case CPS_ERR_DEVICE_NOT_FOUND:       return "device not found";
        case CPS_ERR_PERMISSION:             return "permission denied";
        case CPS_ERR_OPEN:                   return "failed to open device";
        case CPS_ERR_PARITY:                 return "parity error";
        case CPS_ERR_FRAMING:                return "framing error";
        case CPS_ERR_WRITE:                  return "write error";
        case CPS_ERR_READ:                   return "read error";
        case CPS_ERR_RESOURCE:               return "resource error";
        case CPS_ERR_UNSUPPORTED_OPERATION:  return "unsupported operation";
        case CPS_ERR_UNKNOWN:                return "unknown error";
        case CPS_ERR_TIMEOUT:                return "timeout";
        case CPS_ERR_NOT_OPEN:               return "port not open";
    }
