...
public:
    ...
    Impl() = default;
    Impl(std::string pn, std::string desc, std::string manuf, std::string sn,
         std::uint16_t vid, std::uint16_t pid, std::string sysloc)
        : portName(std::move(pn)), description(std::move(desc)), manufacturer(std::move(manuf)),
          serialNumber(std::move(sn)), systemLocation(std::move(sysloc)),
          vendorId(vid), productId(pid), null(false) {}
};
...
SerialPortInfo::~SerialPortInfo() { impl().~Impl(); }
...
SerialPortInfo::SerialPortInfo(const std::string& portName,
    ...
    ::new (mStorage) Impl(portName, description, manufacturer, serialNumber,
                          vendorId, productId, systemLocation);
}
...
std::string SerialPortInfo::portName() const       { return impl().portName; }
std::string SerialPortInfo::description() const    { return impl().description; }
std::string SerialPortInfo::manufacturer() const   { return impl().manufacturer; }
std::string SerialPortInfo::serialNumber() const   { return impl().serialNumber; }
std::string SerialPortInfo::systemLocation() const { return impl().systemLocation; }
std::uint16_t SerialPortInfo::productIdentifier() const { return impl().productId; }
std::uint16_t SerialPortInfo::vendorIdentifier() const  { return impl().vendorId; }
bool SerialPortInfo::isNull() const { return impl().null; }
...
std::vector<SerialPortInfo> SerialPortInfo::availablePorts() {
    ...
    for (const detail::RawPortInfo& r : raw) {
        out.push_back(SerialPortInfo(r.portName, r.description, r.manufacturer, r.serialNumber,
                                     r.vendorId, r.productId, r.systemLocation));
    }
    return out;
}
